...
# LLM (tool-calling can require two LLM round-trips; allow enough time per call)
OLLAMA_CHAT_TIMEOUT = _env_int("OLLAMA_CHAT_TIMEOUT", 90)
OLLAMA_SUMMARY_TIMEOUT = _env_int("OLLAMA_SUMMARY_TIMEOUT", 30)
RECENT_MESSAGES_COUNT = _env_int("RECENT_MESSAGES_COUNT", 40)
SUMMARY_EVERY_MESSAGES = _env_int("SUMMARY_EVERY_MESSAGES", 50)
ENABLE_SUMMARY = os.environ.get("ENABLE_SUMMARY", "true").lower() not in ("0", "false", "no")
# Debug: send no conversation history to LLM (only system + current user message)
DEBUG_NO_HISTORY = os.environ.get("DEBUG_NO_HISTORY", "").lower() in ("1", "true", "yes")

# DB
MAX_HISTORY_MESSAGES = _env_int("MAX_HISTORY_MESSAGES", 300)
